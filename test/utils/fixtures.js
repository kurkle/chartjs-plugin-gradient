 ...
 *
 * Unlike the multi-package layout this was ported from, this repository's 37
 * fixtures sit directly under `test/fixtures/` with no subdirectory, so
 * `specsFromFixtures()` takes no path and always registers every fixture.
 */
...
/** Returns a suite body registering one spec per fixture. */
export function specsFromFixtures() {
  const names = Object.keys(fixtures).sort()

  return () => {
    if (names.length === 0) {
      throw new Error('No fixtures found under test/fixtures')
    }
